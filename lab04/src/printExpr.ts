...
type ParentOperator = BinaryOperator | 'negate';
...
const operatorPrecedence: Record<BinaryOperator | 'negate', number> = {
    add: 1,
    sub: 1,
    mul: 2,
    div: 2,
...
function render(expr: Expr, parentOp?: ParentOperator, position?: ChildPosition): string {
    let printed: string;
...
function needsParenUnderNegate(child: Expr): boolean {
...
function needsParenUnderBinary(parentOp: BinaryOperator, child: Expr, position: ChildPosition): boolean {
    if (child.type !== 'binary') {
        return false;
    }
