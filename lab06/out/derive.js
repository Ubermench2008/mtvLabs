...
const negate = (expr) => {
    ...
    return { type: "negate", operand: expr };
};
const add = (left, right) => binary("add", left, right);
const sub = (left, right) => binary("sub", left, right);
const mul = (left, right) => binary("mul", left, right);
const div = (left, right) => binary("div", left, right);
function derive(e, varName) {
    const derived = deriveNode(e, varName);
    return expandAndCollect(derived);
}
function deriveNode(e, varName) {
    switch (e.type) {
        case "number":
            return number(0);
        case "variable":
...
function deriveBinary(node, varName) {
...
function expandAndCollect(expr) {
...
function exprToPolynomial(expr) {
...
function numberToPolynomial(value) {
    const poly = new Map();
    if (value !== 0) {
...
function variableToPolynomial(name) {
    const poly = new Map();
...
function addPolynomialTerm(result, coeff, factors, displayFactors, alreadySorted = false) {
    if (coeff === 0) {
        return;
    }
...
function polynomialToExpr(poly) {
...
function buildLeadingTerm(term) {
    if (term.coeff >= 0) {
...
function buildPositiveTerm(coeff, orderedFactors) {
    let expr = null;
    if (coeff !== 1 || orderedFactors.length === 0) {
        expr = number(coeff);
    }
    for (const factor of orderedFactors) {
        const factorExpr = { type: "variable", name: factor };
