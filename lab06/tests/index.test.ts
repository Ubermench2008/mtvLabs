...
describe("Advanced simplifications", ()=>{
    test("Collects like terms for square", 5, parseAndDerive, parseExpr("2*x"), "x*x", "x");
    test("Expands product of sums", 5, parseAndDerive, parseExpr("2*x+2*y"), "(x+y)*(x+y)", "x");
    test("Collapses multi-factor derivatives", 5, parseAndDerive, parseExpr("2*x*y"), "x*x*y", "x");
    test("Combines numeric constants after expansion", 5, parseAndDerive, parseExpr("5+2*x"), "(x+2)*(x+3)", "x");
});
