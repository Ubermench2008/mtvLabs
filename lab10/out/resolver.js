...
function buildFunctionTable(module) {
    ...
    for (const func of module.functions) {
        if (table.has(func.name)) {
            throw new src_1.FunnyError(`Function '${func.name}' redeclared`, 'FunctionRedeclaration');
        }
        table.set(func.name, {
...
function declareVariables(defs, scope) {
    for (const def of defs) {
        if (scope.has(def.name)) {
            throw new src_1.FunnyError(`Variable '${def.name}' redeclared`, 'VariableRedeclaration');
        }
...
function validateAssignment(statement, scope, functions) {
    ...
    if (statement.targets.length !== valueTypes.length) {
        throw new src_1.FunnyError('Assignment target count does not match expression', 'TypeMismatch');
    }
    statement.targets.forEach((name, index) => {
        const targetType = scope.get(name);
        if (!targetType) {
            throw new src_1.FunnyError(`Undeclared identifier '${name}'`, 'UndeclaredIdentifier');
        }
        const sourceType = valueTypes[index];
        if (targetType !== sourceType) {
            throw new src_1.FunnyError(`Type mismatch in assignment to '${name}'`, 'TypeMismatch');
        }
    });
}
function inferExpression(expression, scope, functions) {
    switch (expression.type) {
        ...
        case 'call': {
            ...
            if (!signature) {
                throw new src_1.FunnyError(`Call to undeclared function '${expression.callee}'`, 'UndeclaredFunction');
            }
            if (signature.params.length !== expression.args.length) {
                throw new src_1.FunnyError(`Function '${expression.callee}' expects ${signature.params.length} argument(s)`, 'ArgumentMismatch');
            }
            expression.args.forEach((arg, index) => {
                ...
                if (actual !== expected) {
                    throw new src_1.FunnyError(`Argument ${index + 1} of '${expression.callee}' must be of type ${expected}`, 'ArgumentMismatch');
                }
            });
    ...
    switch (predicate.type) {
        ...
        case 'exists': {
            if (scope.has(predicate.variable.name)) {
                throw new src_1.FunnyError(`Variable '${predicate.variable.name}' redeclared`, 'VariableRedeclaration');
            }
...
function ensureInt(types, message) {
    if (expectSingleValue(types, message) !== 'int') {
        throw new src_1.FunnyError(message, 'TypeMismatch');
    }
}
