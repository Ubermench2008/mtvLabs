import { BinaryOperator, Expr } from "../../lab04";
import { cost } from "./cost";

type PathStep = 'left' | 'right' | 'operand';
...
export function simplify(expr: Expr, identities: [Expr, Expr][]): Expr {
    ...
    const visited = new Set<string>([exprKey(expr)]);

    const initialCost = cost(expr);
...
function buildRewrites(identities: [Expr, Expr][]): Rewrite[] {
...
function maybeAddRewrite(rewrites: Rewrite[], pattern: Expr, replacement: Expr) {
...
function collectVariables(expr: Expr, result: Set<string> = new Set()): Set<string> {
    switch (expr.type) {
        case 'number':
            return result;
        case 'variable':
...
function produceNeighbours(expr: Expr, rewrites: Rewrite[]): Expr[] {
    const result: Expr[] = [];
    const seen = new Set<string>();

    const dfs = (node: Expr, path: PathStep[]) => {
...
function evaluateConstant(expr: Expr): number | undefined {
    switch (expr.type) {
        case 'number':
            return expr.value;
        case 'variable':
...
function evaluateBinary(op: BinaryOperator, left: number, right: number): number | undefined {
    switch (op) {
        case 'add':
            return left + right;
        case 'sub':
...
function substitute(pattern: Expr, bindings: Map<string, Expr>): Expr {
    switch (pattern.type) {
        case 'number':
...
function replaceAt(expr: Expr, path: PathStep[], replacement: Expr): Expr {
    if (path.length === 0) {
