...
exports.reprBuffer = reprBuffer;
exports.repr = repr;
exports.BufferedWriter = BufferedWriter;
exports.strRepr = strRepr;
exports.strReprBuffer = strReprBuffer;
...
const style = (str, style) => '\x1B[' + style + 'm' + str + '\x1B[0m';
const reprop = (op) => style(info_1.opcodes[op], '96');
const styleType = (s) => style(s, '33');
...
function visitCell(n, tname, c, depth) {
    c.write(depth, '(' + tname);
    visitAll(n.v, c, depth + 1);
    c.write(depth, ')');
}
function visitValue(v, tname, c, depth) {
    const s = String(v);
...
function visit(n, c, depth) {
    ...
    switch (n.t) {
        ...
        case ast_1.t.instr_imm1: {
            c.write(depth, '(' + reprop(n.v) + ' [');
            visit(n.imm, c, depth + 1);
            return c.write(depth, '])');
        }
        case ast_1.t.instr_pre: {
            c.write(depth, '(' + reprop(n.v));
            visitAll(n.pre, c, depth + 1);
            return c.writeln(depth, ')');
        }
        case ast_1.t.instr_pre1: {
            c.write(depth, '(' + reprop(n.v));
            visit(n.pre, c, depth + 1);
            return c.writeln(depth, ')');
        }
        case ast_1.t.instr_imm1_post: {
            c.write(depth, '(' + reprop(n.v) + ' [');
            visit(n.imm, c, depth + 1);
            c.write(depth, ']');
        ...
        case ast_1.t.instr_pre_imm_post: {
            c.write(depth, '(' + reprop(n.v) + ' [');
            visitAll(n.imm, c, depth + 1);
            c.write(depth, ']');
            visitAll(n.pre, c, depth + 1);
        ...
        case ast_1.t.import_entry:
        case ast_1.t.export_entry:
        case ast_1.t.local_entry:
        case ast_1.t.table_type:
        case ast_1.t.memory_type: {
            return visitCell(n, tname, c, depth);
        }
        case ast_1.t.func_type: {
            // func, paramCount, paramT*, retCount, retT* -> "(" paramT* ")" retT
            c.write(depth, '(' + tname + ' (');
            const paramCount = n.v[1].v;
            const resCount = n.v[1 + paramCount + 1].v;
        ...
        case ast_1.t.global_type: {
            c.write(depth, '(' + tname);
            visitAll(n.v.slice(0, n.v.length - 1), c, depth + 1);
            c.write(depth + 1, (n.v[1].v ? ' mutable' : 'immutable'));
            return c.write(depth, ')');
        }
        case ast_1.t.resizable_limits: {
            return c.write(depth, '(' + tname + ' ' + n.v[1].v + '..' + (n.v[0].v ? n.v[2].v : '') + ')');
        }
        case ast_1.t.global_variable:
        case ast_1.t.init_expr:
        case ast_1.t.elem_segment:
        case ast_1.t.data_segment:
        case ast_1.t.function_body: {
            return visitCell(n, tname, c, depth);
        }
        case ast_1.t.str: {
            return c.write(depth, '"' + utf8_1.utf8.decode(n.v) + '"');
        }
        default:
            throw new Error('unexpected type ' + n.t.toString());
    }
}
function reprBuffer(buffer, w, limit, highlightRange, options) {
...
function repr(n, w, options) {
    ...
    if (!options || options.colors !== false) {
        ...
        let immOpen = style('[', '2'), immClose = style(']', '2');
        writer = s => {
            w(s.replace(/([^\x1B]|^)[\[\]]/g, m => m.length == 2 ?
...
function BufferedWriter() {
    const buf = [];
    const writer = (s) => { buf.push(s); };
    writer.toString = () => buf.join('');
    return writer;
}
...
function strRepr(n, options) {
    const w = BufferedWriter();
    repr(n, w, options);
    return w.toString();
}
function strReprBuffer(buffer, limit, highlightRange, options) {
    const w = BufferedWriter();
